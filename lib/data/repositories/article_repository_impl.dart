...
import 'package:application_berita_b/common/network_info.dart';
import 'package:application_berita_b/data/datasources/db/article_local_data_source.dart';
...
import 'package:application_berita_b/common/exception.dart';
  ...
  Future<Either<Failure, List<Article>>> getHeadlineBusinessArticles() async {
    if (await networkInfo.isConnected) {
      try {
        ...
        localDataSource.cacheHeadlineBussinesArticles(
            result.map((article) => ArticleTable.fromDTO(article)).toList());
        return Right(result.map((model) => model.toEntity()).toList());
      } on ServerException {
        return Left(ServerFailure(''));
      ...
      try {
        final result =
            await localDataSource.getCacheHeadlineBussinesArticles();
        return Right(result.map((model) => model.toEntity()).toList());
      } on CacheException catch (e) {
        return Left(CacheFailure(e.message));
      }
