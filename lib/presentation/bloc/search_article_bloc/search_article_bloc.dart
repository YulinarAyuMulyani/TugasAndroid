...
import 'package:application_berita_b/domain/usecase/search_article.dart';
      ...
      if (query.isEmpty) {
        ...
        result.fold((failure) => emit(SearchArticleError(failure.message)),
          ...
          emit(SearchArticleHasData(
              articlesData.articles, articlesData.totalResult, 1));
    ...
    on<OnNextPage>((event, emit) async {
      ...
      if (query.isEmpty) {
        ...
        result.fold((failure) => emit(SearchArticleError(failure.message)),
          ...
          emit(SearchArticleHasData(articles, articleData.totalResult, page));
